...
import model
import train
import network  # Importing your Network class module
...
# Create network using protein p-values from CSV
def create_network_from_proteins(p_value_df, kge):
    graph = network.Network(p_value_df, kge)
    return graph
...
def create_embedding_with_proteins(p_value=0.05, save=True, data_dir='gat/data/emb'):
    ...
    proteins_train, proteins_test = train_test_split(filtered_df, test_size=0.3, random_state=42)
