...
def create_network_from_proteins(data, kge):
    graph = Network('gat/data/protein_interaction_p_value_results_with_fdr_975_stId_name.csv')  # Initialize the protein network
...
def create_embedding_with_proteins(p_value=0.05, save=True, data_dir='gat/data/emb'):
    ...
    p_value_path = os.path.join('gat/data/', 'protein_interaction_p_value_results_with_fdr_975_stId_name.csv')
...
def create_embeddings(load_model=True, save=True, data_dir='gat/data/emb', hyperparams=None, plot=True):
    ...
    net = model.GCNModel(dim_latent=out_feats, num_layers=num_layers).to(device)
