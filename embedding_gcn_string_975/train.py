...
def train(hyperparams=None, data_path='gat/data/emb', plot=True):
    ...
    for data in dl_train:
        ...
        csv_save_path_initial = os.path.join('gat/data/', f'protein_embeddings_lr{learning_rate}_dim{out_feats}_lay{num_layers}_epo{num_epochs}_initial.csv')
    ...
    for data in dl_train:
        ...
        csv_save_path_final = os.path.join('gat/data/', f'protein_embeddings_lr{learning_rate}_dim{out_feats}_lay{num_layers}_epo{num_epochs}_final.csv')
        ...
        save_protein_embeddings_initial = os.path.join(results_path, f'protein_embeddings_lr{learning_rate}__dim{out_feats}_lay{num_layers}_epo{num_epochs}_initial.csv')
        protein_embeddings_initial.to_csv(save_protein_embeddings_initial, index_label='stId')

        protein_embeddings_final = pd.DataFrame.from_dict(all_embeddings)##, orient='index')
        save_protein_embeddings_final = os.path.join(results_path, f'protein_embeddings_lr{learning_rate}__dim{out_feats}_lay{num_layers}_epo{num_epochs}_final.csv')
        protein_embeddings_final.to_csv(save_protein_embeddings_final, index_label='stId')  ''' 
...
def plot_cosine_similarity_matrix_for_clusters_with_values(embeddings, stids, save_path):
    ...
    ax.text(x=0.5, y=-0.03, s="Protein-protein similarities", fontsize=12, ha='center', va='top', transform=ax.transAxes)
