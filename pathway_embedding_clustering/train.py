...
def train(hyperparams=None, data_path='embedding/data/emb', plot=True):
    ...
    save_embeddings_to_csv(all_embeddings, stid_list)
...
def save_embeddings_to_csv(embeddings, stid_list, filename="gat/data/pathway_embeddings.csv"):
    # Create a DataFrame with the node stIds and corresponding embeddings
    df_embeddings = pd.DataFrame(embeddings, index=stid_list)
    df_embeddings.index.name = "stId"
    df_embeddings.columns = [f"embedding_{i+1}" for i in range(df_embeddings.shape[1])]
    
    # Save to CSV
    df_embeddings.to_csv(filename)
